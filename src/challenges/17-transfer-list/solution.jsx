...
export default function TransferList({ initialLeft, initialRight }) {
  ...
  const renderList = (items, label) => <div style={{ border: "1px solid #d1d5db", borderRadius: "4px", padding: "8px", minWidth: "150px" }}>
      <h3>{label}</h3>
      {items.map((item) => <label key={item} style={{ display: "block", padding: "4px 0" }}>
          <input
    type="checkbox"
