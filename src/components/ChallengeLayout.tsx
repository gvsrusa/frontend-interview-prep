...
import { getReactChallenges } from '@/lib/registry'
...
type RightTab = 'preview' | 'editor' | 'solution'

export default function ChallengeLayout() {
  const { slug } = useParams<{ slug: string }>()
  const challenges = useMemo(() => getReactChallenges(), [])
  ...
  const ext = activeLang === 'ts' ? 'tsx' : 'jsx'
  const indexPath = entry ? `challenges/${slug}/index.${ext}` : ''
  const solutionPath = entry ? `challenges/${slug}/solution.${ext}` : ''
  ...
  const { Component: PreviewComponent, error: previewError, compiling, version: previewVersion } = usePreviewComponent(editorCode, ext)
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            <div className="min-h-[400px]">
              {activeTab === 'preview' && (
                <div className="rounded-xl border border-border bg-surface-light p-6 min-h-[400px]">
                  {previewError ? (
                    <div className="rounded-lg border border-hard/30 bg-hard/10 p-4">
                      <p className="text-hard font-semibold text-sm mb-2">Compile Error</p>
                      <pre className="text-xs text-hard/80 whitespace-pre-wrap">{previewError}</pre>
                    </div>
                  ) : PreviewComponent ? (
                    <ErrorBoundary resetKey={previewVersion}>
                      <div className="rounded-lg bg-white p-4 text-gray-900">
                        <PreviewComponent />
                      </div>
                    </ErrorBoundary>
                  ) : compiling ? (
                    <p className="text-text-muted text-sm">Compiling...</p>
                  ) : editorLoading ? (
                    <p className="text-text-muted text-sm">Loading...</p>
                  ) : (
                    <p className="text-text-muted text-sm italic">Write your component in the Code Editor tab to see a live preview here.</p>
                  )}
                </div>
              )}
